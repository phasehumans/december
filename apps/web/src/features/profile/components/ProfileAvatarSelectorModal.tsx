...
export const ProfileAvatarSelectorModal: React.FC<ProfileAvatarSelectorModalProps> = ({
    ...
    return createPortal(
        ...
        >
            <div
                ref={modalRef}
                className="w-full max-w-[480px] bg-[#141414] border border-[#242323] rounded-[20px] shadow-2xl relative p-6 flex flex-col mx-4 animate-in zoom-in-95 duration-200"
            >
                    ...
                    {AVATAR_SEEDS.map((seed) => (
                        <button
                            ...
                            className={`relative aspect-square rounded-full border-[3px] transition-all overflow-hidden ${
                                selected === seed
                                    ? 'border-[#D6D5C9] scale-105 shadow-[0_0_15px_rgba(214,213,201,0.15)] bg-[#191919]'
                                    : 'border-[#2B2A29] bg-[#100E12] hover:border-[#4A4948] hover:bg-[#191919] hover:scale-105'
                            }`}
                        >
                            <img
                            ...
                            {selected === seed && (
                                <div className="absolute inset-0 bg-black/40 flex items-center justify-center backdrop-blur-[1px]">
                                    <div className="w-7 h-7 bg-[#D6D5C9] rounded-full flex items-center justify-center shadow-xl">
                                        <Check className="w-4 h-4 text-[#141414]" strokeWidth={3} />
                                    </div>
                                </div>
                            )}
                        </button>
                ...
                <div className="flex items-center justify-end gap-3 mt-auto">
                    <button
                        onClick={onClose}
                        className="px-5 py-2 rounded-xl border border-[#383736] text-[13.5px] font-medium text-[#D6D5C9] hover:bg-[#191919] transition-colors"
                    >
                        Cancel
                    </button>
                    <button
                        onClick={handleSave}
                        className="px-5 py-2 rounded-xl bg-[#D6D5C9] text-[#141414] text-[13.5px] font-medium hover:bg-white transition-colors"
                    >
                        Save changes
                    </button>
                </div>
