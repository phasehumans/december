...
describe('Provider Switch Logic (Unit)', () => {
    it('lists all configured providers across subscriptions, wallet, and BYOK', () => {
        ...
        expect(subItem?.label).toContain('Claude (Subscription)')
        ...
        expect(walletItem?.label).toContain('December (Cloud Wallet)')
        ...
        expect(antItem?.label).toContain('Anthropic (API Key)')
