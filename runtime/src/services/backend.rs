...
use crate::{
    app::config::BackendCallbackConfig,
    app::state::{PreviewBackendStatus, PreviewStatusSnapshot},
...
struct BackendStatusPayload<'a> {
    ...
    state: &'a crate::app::state::PreviewLifecycleState,
    current_version: Option<&'a str>,
    healthy_version: Option<&'a str>,
    preview_url: Option<&'a str>,
    error: Option<&'a crate::domain::error::StructuredError>,
...
impl BackendCallbackClient {
    ...
    pub async fn report_status(
        ...
        if !response.status().is_success() {
            ...
            warn!(status = %status, body = %body, "backend callback returned non-success status");
        }
