...
use crate::{
    actor::{PreviewActor, PreviewActorHandle},
...
impl ActorRegistry {
    ...
    pub async fn status(&self, preview_id: &str) -> Result<PreviewStatusSnapshot, RuntimeServiceError> {
    ...
    async fn ensure_actor(&self, preview_id: String, project_id: String) -> PreviewActorHandle {
        {
            let guard = self.inner.read().await;
