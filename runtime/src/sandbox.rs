...
use crate::{config::DockerConfig, error::RuntimeServiceError};
...
impl DockerSandbox {
    ...
    async fn run_shell(
        ...
        match self.docker.start_exec(&exec.id, None).await.map_err(|error| {
            RuntimeServiceError::infra_runtime(
                "failed to start Docker exec",
                Some(error.to_string()),
            )
        })? {
            StartExecResults::Attached { mut output: logs, .. } => {
    ...
    async fn stop_dev_server_process(&self, container_name: &str) -> Result<(), RuntimeServiceError> {
        let _ = self
            .run_shell(
                container_name,
...
impl Sandbox for DockerSandbox {
    async fn ensure_started(&self, workspace_host_path: &Path) -> Result<(), RuntimeServiceError> {
        ...
        exposed_ports.insert(format!("{}/tcp", self.config.container_port), HashMap::new());
    ...
    async fn install_dependencies(&self) -> Result<(), RuntimeServiceError> {
        ...
        let (exit_code, output) = self
            .run_shell(&container_name, "cd /workspace && bun install --no-progress")
            .await?;
    ...
    fn preview_target_url(&self) -> Option<String> {
        ...
        state.host_port.map(|port| format!("http://127.0.0.1:{port}"))
    }
}
