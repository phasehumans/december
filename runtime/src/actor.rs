...
use crate::{
    ...
    manifest::{ManifestDiff, ManifestRef, PreviewManifest, ReconcileMode},
    sandbox::{DockerSandbox, Sandbox},
    state::{PreviewLifecycleState, PreviewStatusSnapshot, backend_status_for},
    storage::ObjectStorage,
    workspace::{ensure_workspace, sync_workspace},
};
...
impl PreviewActor {
    pub fn spawn(
        ...
        let sandbox = Arc::new(
            DockerSandbox::new(preview_id.clone(), config.docker.clone())
                .expect("docker sandbox must initialize"),
        );
    ...
    async fn run(mut self) {
        ...
        while let Some(command) = tokio::select! {
            ...
            match command {
                ActorCommand::ApplyManifest(manifest_ref) => {
    ...
    async fn reconcile(&mut self, manifest_ref: ManifestRef) -> Result<(), RuntimeServiceError> {
        let manifest = self.storage.fetch_manifest(&manifest_ref.manifest_key).await?;
        ...
        match diff.mode {
            ReconcileMode::Bootstrap => {
                self.transition(PreviewLifecycleState::Bootstrapping, None).await;
                self.ensure_sandbox_ready().await?;
                self.sync_manifest(&manifest, &diff).await?;
                self.transition(PreviewLifecycleState::Installing, None).await;
            ...
            ReconcileMode::Reinstall => {
                self.transition(PreviewLifecycleState::Rebuilding, None).await;
                self.ensure_sandbox_ready().await?;
                self.sync_manifest(&manifest, &diff).await?;
                self.transition(PreviewLifecycleState::Installing, None).await;
                self.sandbox.install_dependencies().await?;
    ...
    async fn await_healthy(&mut self, manifest: &PreviewManifest) -> Result<(), RuntimeServiceError> {
        ...
        for _ in 0..30 {
            ...
            last_detail = health.detail.clone().or_else(|| health.body_excerpt.clone());
    ...
    async fn fail(&mut self, error: RuntimeServiceError) {
        self.transition(PreviewLifecycleState::Failed, Some(error)).await;
    }
...
impl PreviewActorHandle {
    pub async fn apply_manifest(&self, manifest: ManifestRef) -> Result<(), RuntimeServiceError> {
        self.command_tx.send(ActorCommand::ApplyManifest(manifest)).await.map_err(|error| {
    ...
    pub async fn stop(&self) -> Result<(), RuntimeServiceError> {
        self.command_tx.send(ActorCommand::Stop).await.map_err(|error| {
            RuntimeServiceError::infra_runtime(
                "preview actor is unavailable",
                Some(error.to_string()),
            )
