...
if (!process.env.JWT_SECRET) {
    throw new Error('JWT_SECRET is required for auth service integration tests')
}
...
describe('auth.service integration', () => {
    ...
    it('should fallback to emptyusername if getUsernameFromEmail returns empty string', async () => {
        ...
        expect(user).not.toBeNull()
        expect(user!.name).toBe('emptyusername')
        expect(user!.username).toBe('emptyusername')
    })
    ...
    it('should fallback to emptyusername for new google user when username extraction is empty', async () => {
        const token = await authService.google({
            name: 'Digit User',
            email: '123456@example.com',
            sub: 'google-sub-empty-username',
        })

        expect(token).toBeString()

        const user = await prisma.user.findUnique({
            where: { email: '123456@example.com' },
        })
    ...
    it('should mark existing local user as verified when attaching googleId (recommended behavior)', async () => {
        const email = 'local-unverified@example.com'
        const sub = 'google-sub-local-unverified'
